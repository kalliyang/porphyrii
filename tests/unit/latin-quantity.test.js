...
test("derive: Aeneid 1.1 weight sequence matches the frozen fixture", () => {
  // – ⏑⏑ | – ⏑⏑ | – – | – – | – ⏑⏑ | – x
  assert.equal(
...
test("derive: weight follows macrons, not lexical inference", () => {
...
test("derive: mute+liquid is light by default and flagged", () => {
...
test("hexameter: Aeneid 1.1 matches the frozen feet", () => {
...
test("hexameter: ml-indeterminate syllable fits either slot", () => {
...
test("pentameter: Catullus 85.2 matches with central longum", () => {
  // Closed-syllable analysis: Nēs-cĭ-ŏ | sēd fĭ-ĕ | rī ‖ sēn-tĭ-ĕ(t) | tēx-crŭ-cĭ | ōr
...
test("validateScansion: solver short against a macron → mismatch", () => {
  const bad = structuredClone(AEN11_CONTRACT);
...
test("validateScansion: syllable-count divergence reported as structural note", () => {
  ...
  // explicit (empty) overrides bypass the syllable-override transport so the raw count
...
test("validateScansion: blank physical lines do not misalign entries", () => {
  const mk = (lineNo) => ({
    line: lineNo,
    text: "māla",
    feet: [
...
test("validateScansion: line field must equal array position (fail-closed)", () => {
  const mk = (lineNo) => ({
    line: lineNo,
    text: "māla",
    feet: [
