 ...
 *
 * Verifies the full frontend pipeline against a running site with /api/*
 * responses fulfilled by deterministic mocks. Cloudflare Turnstile refuses
 * automated browsers by design, so no automated run can pass the real
 * challenge. Here the
...
// E: declared spelling correction (warning path, the integrity-warning behavior). A letter is
// dropped so Check B sees a real diff (v/u variants normalize away).
...
(async () => {
  ...
  // ---- Aen. 1.3 double elision + liaison.
