...
> 例句取自已核实出处的公版原文；macron 为词汇级标注草案。
...
- 对照反例（suus 族必须排除）：**suam/suae/suō 等为两音节 [sʊ.am]**——例句槽位留空，供学习者从任意叙事文本自行补充；规则要点：「sua」字符串匹配会同时命中 suādeō 族与 suus 族，必须词干级判定。
...
- 出处核实：原文经**两个独立公版语料逐字符核对一致**（Latin Library 本 + 拉丁 Wikisource 本）。
- macron：itaque 全短（ĭ-tă-quĕ）；necessāriīs / negōtiīs / cūrīsque / vacuī / avēmus / vidēre / audīre 按标准词汇量标出。
