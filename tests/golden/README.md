...
| `aeneid-1-1-7.ipa-gold.json` | Aen. I.1–7 IPA gold samples | Lab derivation under the frozen rule table (G2P.md v1.0.2 §10.2); macron layer identical to Chamberlain. `gold_ipa_md` is verbatim from the frozen markdown; `expected_ipa` is the rule-canonical rendering. Three notation errata in the frozen md are documented per line (L1/L3 dropped syllable dots, L2 lax notation) — reported for adjudication, not silently patched |
