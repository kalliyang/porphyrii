 ...
 *
 * Implements the v0.1.1 public driver contract. Language-agnostic: no
 ...
 *
 * Engine initialization and synthesis run on the main thread.
 *
 * Usage:
...
const DEFAULTS = { rate: 175, pitch: 50 };
...
/* ----------------------------------------------------------------- state
 *
 * init() is a single-flight operation. Concurrent callers
...
/* ---------------------------------------------------- mapping table load
 *
 * Default: ./la.json alongside this driver, matching the flat release layout.
 * Node.js: read via fs (undici fetch has no file: support); mappingURL then
 * accepts a plain filesystem path, a file: URL, or an http(s) URL, using the
 * same convention as wasmURL/dataURL.
 */
...
/* kind is required: stress repositioning attaches pending stress marks
 ...
 * whole NFC input. UnmappableSymbolError.position is the codepoint index in
 * the NFC input, whitespace spans included; an astral codepoint counts as one
 * position, not two UTF-16 units.
 *
...
export async function init(options) {
    ...
    if (_initPromise) return _initPromise; // concurrent init shares the flight
...
export function terminate() {
    _generation++; // invalidate any in-flight init
