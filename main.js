/**
 * main.js — the orchestrator, and the ONLY DOM-manipulation layer (the frontend architecture).
 *
 ...
 *   app/audio.js           recitation state machine and driver loader
 *   app/turnstile.js       invisible-widget wrapper (fresh token per request)
 *   app/db.js              IndexedDB history
 *   services/text-integrity.js   two-level check + diff (isomorphic)
 *   core/latin-quantity.js      quantity self-consistency validator
 *
 * LLM output is NEVER injected as HTML (the safe-rendering policy): every string goes
 * through textContent.
 */
...
/** Plain-text block -> <p> per blank-line-separated paragraph (the safe-rendering policy). */
...
// ---------------------------------------------------------------------------
// Analysis state machine (the challenge behavior)
// ---------------------------------------------------------------------------
...
function renderResult(contract, { input, save }) {
  ...
  if (contract.spelling_corrected === true) {
    // Expected-correction mismatch renders as WARNING, not error (the integrity-warning behavior).
...
// ---------------------------------------------------------------------------
// Recitation state machine and driver integration
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Offline banner: explicit degradation, never silent
// ---------------------------------------------------------------------------
